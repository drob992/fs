...
class LiveEvent(QWebPage):
	...
	def open_sport_group(self):
		...
		main = self._frame.findFirstElement("#page")
		...
		for x in range(len(events)):
			if events.at(x).attribute('data-alive-sport-match') == self.tipbet_sport_id:
				...
				if self._event_hash == str(ev_hash):
					...
					# util.simulate_click(events.at(x).findAll(".col20").at(0).findAll("span").at(0))
	...
	def parse_event(self):
		...
		# self.delay_kickoff_log = True
		# self.three_time_open_event("remove_flag")
		main = self._frame.findFirstElement('#page')
		# self._temp_removed_match['number_of_tries'] = 0

		# self.parse += 1
		# if self.statistics_cd < 2:
		# 	self.statistics_cd += 1

		all_events = main.findAll(".green")
		for event in range(len(all_events)):

			btn_quotas = all_events.at(event).findAll(".col20").at(0).findAll("span").at(0)
			print("qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq")
			print(all_events.at(event).attribute("id"), self.event_id)
			if all_events.at(event).attribute("id") == self.event_id:
				print("stefan stefan stefan stefan stefan stefan stefan")
				if not btn_quotas.hasClass("selected"):
					util.simulate_click(btn_quotas)
			else:
				all_events.at(event).removeAllChildren()

		#self.parse je brojac koji odlaze parse_event na pocetku za 3 sekunde zarad davanja vremena ucitavanju strane
		...
		#
		# 	if self._statistics_availability == 10:
		# 		self.log.info("\nStatistik availability brojac je otisao na 20, resetujemo stranu")
		# 		util.hide_quotas_on_liveboard(event_hash=self._event_hash)
		# 		self.reload_single_page()
		#
		# 	#ako statistika ne valja, reloadujemo stranu
		# 	if not self.statistics_availability():
		# 			self.reload_single_page()
		#
		# 	self.EVENT.setStartTime('None')
		#
		# 	quota_table = main.findAll(".ip-MobileEventView_GridWrapper ").at(0).toPlainText().strip()
		# 	self.old_quota_hash_set = self.quota_hash_set
		# 	self.quota_hash_set = util.hash('TIPBETLIVE' + quota_table)
		#
		#
			if self._event_sport == 'Football':
				...
				if util.football_time_change_checker(main, self.redis, self.EVENT.time, self.log, self._event_hash):
					if util.football_time_jump(main, self.EVENT.time, self.log, self._event_hash, self.redis):
						if util.football_annulled_score(main, self.EVENT.current_score, self.log, self._event_hash, self.EVENT.team1, self.EVENT.team2):
							self.EVENT.setTime(main, self._event_hash, rdb=self.redis)
							self.EVENT.setScore(main)
							self.EVENT.setRedCards(main)
							self.EVENT.setYellowCards(main)
					...
					######## ISKLJUCI KVOTE
					self.log.critical("minut se nije promenio vise od 120 sekundi {} {}!!!!!!!!!!!!!!!!!!!!!!!!!!\n".format(self.EVENT.time, ev_hash))
					print("minut se nije promenio vise od 120 sekundi {} {}!!!!!!!!!!!!!!!!!!!!!!!!!!\n".format(self.EVENT.time, ev_hash))
					util.hide_quotas_on_liveboard(event_hash=self._event_hash)
					self.take_screenshot()
					self.reload_single_page()

		# 	if self._event_sport not in ['Tennis', 'Volleyball']:
		# 		if self.quota_hash_set != self.old_quota_hash_set or self.quota_hash_set_flag == 10:
		# 			#Proveravamo kvote samo ako su hashevi rasliciti ili ako je brojac dosao do 10
		# 			self.EVENT.setOdds(main)
		# 			self.quota_hash_set_flag = 0
		# 		else:
		# 			#Svake sekunde ako je hash isti, brojac ide +1
		# 			self.quota_hash_set_flag += 1
		#
		# 	emmited_ft_key = 'emmited_ft_{}'.format(self._event_hash)
		#
		# 	data = None
		#
		# 	if self.EVENT.time != 0 and \
		# 		self.EVENT.time != 'FT' and \
		# 		self.EVENT.time != 'removed' and \
		...
		# 		self.redis.set("bet365_last_emmit-{}".format(self._event_hash), timestamp)
